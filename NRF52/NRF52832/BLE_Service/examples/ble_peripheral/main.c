...
static void TIME_update(void) //上传数据给手机，本例程是定时器自增一 ，2秒更新一次
{
	...
	ble_led_value_updata(&m_ls,TIME,BLE_CONN_HANDLE_ALL);
...
//LED写事件处理
static void led_write_handler(uint16_t conn_handle, ble_ls_t * p_lbs, uint8_t led_state)
{
    if (led_state)
    {
...
static void services_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		...
		ls_init.led_write_handler = led_write_handler;
		ls_init.p_report_ref = NULL;
		ls_init.support_notification = true;
//		ls_init.initial_batt_level = 0;
		
		ls_init.bl_rd_sec = SEC_OPEN;
		ls_init.bl_cccd_wr_sec = SEC_OPEN;
		ls_init.bl_report_rd_sec = SEC_OPEN;
	
		err_code = ble_ls_init(&m_ls, &ls_init);
		APP_ERROR_CHECK(err_code);
